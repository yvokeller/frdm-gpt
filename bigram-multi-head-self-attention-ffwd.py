...
class BigramLanguageModel(nn.Module):
    def __init__(self):
        ...
        # feed-forwrard layer - allow the tokens to think on what they found from the other tokens!
        self.ffwd = FeedFoward(n_emb_d)
    ...
    def forward(self, idx, targets=None):
        ...
        x = self.ffwd(x) # (B,T,C) | note: this is run on a per-token level! self-attention is run on a per-sequence level and allowed the tokens to interact with each other
